...
Всем привет! Все мы здесь бьемся над одной целью — стать Java Developer-ами.
...
В этой статье мы поговорим об одной из таких ключевых тем — базы данных. Рассмотрим наиболее частые вопросы и попытаемся ответить на них без глубокого погружения в материал, ведь в таком случае нам и объема книги не хватит!

Итак, поехали.Собеседование разработчика: разбор вопросов по базам данных - 1
...
Нереляционные системы (NoSQL) — системы, созданные для определенных моделей данных, обладающие гибкими схемами. Иначе говоря, это БД, которые хранят данные не в виде табличных схем, строк и столбцов, а в иных форматах.
Подробнее о нереляционных базах данных можно прочесть в этой статье: Руководство по NoSQL для разработчиков.
 
Система управления базами данных (СУБД) — это совокупность программного обеспечения, при помощи которого пользователь может создавать базы данных (БД) и проводить над ними различные операции: дополнять, обновлять, удалять, выбирать, и т. д. 

СУБД гарантирует сохранность, целостность, безопасность хранения данных и позволяет выдавать доступ к администрированию БД.

В качестве примера можно привести MySql — это СУБД, предоставляющая доступ к реляционной базе данных или MongoDB для нереляционной.
...
Нормальная форма — свойство таблицы, рассматриваемое в контексте нормализации, которое характерезует таблицу с точки зрения простоты и правильности построения структуры. Нормальная форма определяется как совокупность требований, которым должна удовлетворять таблица. 
...
Выше мы говорили о кластеризованных и некластеризованных индексах, которые могут быть уникальными.
...
Ничего не напомнило?Собеседование разработчика: разбор вопросов по базам данных - 3Ведь первичный ключ — ничто иное, как уникальный, кластеризованный индекс.
...
При соединении с базой данных при помощи JDBC вы используете различные Statements. Для повышения безопасности необходимо использовать PreparedStatement вместо обычного Statement, так как при использовании Statement строки запроса и значений просто складываются, делая возможными инъекции. 

В свою очередь, в PreparedStatement есть конкретный шаблон запроса, и данные в него вставляются с отражением кавычек. В итоге SQL-инъекции будут восприниматься только как строковое представление какого-то поля.

Для защиты от SQL-инъекций можно использовать проверку на основе регулярных выражений (подробнее о регулярных выражениях можно почитать в этой статье).Собеседование разработчика: разбор вопросов по базам данных - 9Ещё вариант — установить ограничение на количество символов входящих параметров: например, если вам должно приходить число не более 9999, подойдет ограничение в четыре входящих символа. Оно снизит риск взлома с помощью SQL-инъекций.Подробнее о безопасности в Java можно узнать из статьи “Безопасность в Java: best practices”.
Frontend-университет

14. Что такое хранимые процедуры? Хранимые функции? Триггер?
Хранимые процедуры в SQL — сущность в БД, представляющая собой набор SQL-инструкций, который компилируется один раз и хранится на сервере. Словом, это аналог методов в Java.
...
15. Практика
Как бы то ни было, самым частым SQL-вопросом на собеседовании будет практика — решение задач. 
...
В качестве ресурса для практики на различных задачах может послужить sql-ex.ru.

Уже после первых двадцати выполненных тасков вашему собеседнику будет довольно сложно напугать вас какой либо SQL-задачей.Собеседование разработчика: разбор вопросов по базам данных - 11На этом сегодня все: надеюсь, после прочтения данной статьи вопросы по базам данных не вызовут никаких сложностей и проблем.
