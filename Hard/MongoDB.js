...
insert command while not providing an _id: Inserts a new document.
...
SQL Databases: Store data in tabular format with predefined models with limited flexibility for fast-growing apps.
Modern Applications: Social, interactive, and data-heavy applications that require fast access to large volumes of data.
...
Знакомы ли вы с СУБД? Можете ли вы объяснить это?

Знакомы ли вы с RDMS? Можете ли вы это объяснить?

Что такое MongoDB?

В чем разница между нормализацией и денормализацией?

Знакомы ли вы с SQL? Можете ли вы объяснить это?

Что такое компилятор DML?

Что такое функциональная зависимость?

Что такое соединение?

Можете ли вы объяснить операцию с базой данных?

Вопросы об опыте работы с MongoDB и биографии
После того как интервьюеры оценят ваши общие знания в этой области, они могут захотеть узнать больше о вашем опыте и биографии. Следующие вопросы относятся к вашему опыту:

Как долго вы работаете с MongoDB?

Работали ли вы с какими-либо альтернативами MongoDB?

Можете ли вы объяснить, с какой проблемой, связанной с базой данных, вы столкнулись и как вы ее решили?

Работали ли вы когда-нибудь в роли контролера баз данных?? Узнали ли вы что-нибудь новое?

Как долго вы работаете с SQL?

Имеете ли вы опыт работы с MySQL?

Можете ли вы объяснить, с какой проблемой вы столкнулись и как вы ее решили?

Почему вы можете выбрать MongoDB вместо других альтернатив??

Знакомы ли вы с методом pretty()?

Знакомы ли вы с методом limit()?

Углубленные вопросы для собеседования по MongoDB
Узнав больше о вашей биографии, интервьюеры переходят к углубленным вопросам. Именно здесь вы подтверждаете свои знания сложными вопросами, иногда создавая код перед интервьюером. Следующие вопросы относятся к более углубленной теме:

Каков синтаксис создания коллекции в MongoDB?

Какой синтаксис используется для удаления коллекции в MongoDB?

Какую команду вы используете при вставке документа в MongoDB??

Что такое GridFS? Как вы это используете?

Для чего используется журналирование в MongoDB?

Как создать индекс?

Определите и объясните проекцию в MongoDB.

Можете ли вы объяснить ситуацию, в которой вы бы использовали функцию MapReduce?

Когда бы вы использовали функцию блокировки в MongoDB?
...
Для ролей, предполагающих использование кода, интервьюеры предпочитают видеть работу кандидата в действии. На этот вопрос есть простой ответ - просто ввести команду. Будьте готовы к тому, что вам придется столкнуться с подобным вопросом о прикладном ноу-хау:

Пример: db_adminCommand ( connPoolStats )

2. Можете ли вы объяснить и определить компоненты кластера sharded??
Это глубокий вопрос, требующий академического ответа, а не доказательства работы. Перечислите и объясните термины в простой лексике, поскольку не все интервьюеры являются экспертами в этой области.

Пример: В кластере sharded есть три компонента. Первый компонент - это сами шарды. Это наборы реплик, которые содержат подмножества данных, используемые в кластере. Второй компонент - маршрутизатор запросов или процесс mongo, который действует как интерфейсная точка кластера. Последний компонент - это серверы конфигурации, которые хранят метаданные кластера.

3. Можете ли вы дать определение и перечислить команды DDL?
Некоторые вопросы, подобные этому, являются очень общими и предназначены для оценки базовых знаний. Определите, что представляют собой эти команды в основных терминах и перечислите соответствующие команды.

Пример: Команды DDL определяют структуру данных. После выполнения эти команды сохраняются навсегда. Существует пять команд: CREATE, ALTER, TRUNCATE, DROP и RENAME. Каждая команда выполняет то, что следует из ее названия, позволяя программистам создавать, изменять, усекать, удалять или переименовывать таблицы или базы данных.

4. Можете ли вы определить и ввести команды DML?
Как и предыдущий вопрос, этот также оценивает общие знания. Определите, что представляют собой эти команды в основных терминах, и перечислите сами соответствующие команды.

Пример: Команды DML манипулируют данными. Они не сохраняются навсегда, и при необходимости их можно откатить назад. Существует четыре команды: INSERT, UPDATE, DELETE и MERGE. Каждая команда выполняет то, что следует из ее названия, позволяя программистам вставлять, обновлять, удалять или объединять строки в таблицах.

5. Можете ли вы определить типы команд DCL?
Как и предыдущие два вопроса, этот оценивает общие знания, опять же с помощью команд. Определите, что такое эти команды в базовых терминах, и перечислите сами соответствующие команды.

Пример: Команды DCL управляют видимостью данных, например, отзывают разрешения на доступ. Существует три команды: COMMIT, ROLLBACK и SAVEPOINT. Коммиты сохраняются навсегда, а точки сохранения сохраняются только временно.

6. Можете ли вы написать запрос, чтобы получить пятую по величине зарплату из таблицы сотрудников?
Для эффективного ответа на этот вопрос требуется среда кодирования. Интервьюеры часто требуют, чтобы кандидаты доказали перед ними свои навыки кодирования. Тщательно проанализируйте вопрос, принимая во внимание все детали, о которых просит интервьюер, и напишите код.
...
FROM Сотрудник Emp2
ГДЕ Emp2.Сотрудник_Зарплата >= Emp1.Сотрудник_Зарплата_)

7. Можете ли вы дать определение независимости данных и перечислить два типа?
Это такой же вопрос, как и любой другой вопрос по словарному запасу. Дайте определение простым языком. Чтобы произвести еще большее впечатление на интервьюера, определите эти два типа, а не просто перечислите их.

Пример: Независимость данных означает способность изменять схему на одном уровне, не затрагивая схему на другом. Позволяет изменять определенные наборы данных без изменения других, редактируя каждый уровень независимо друг от друга.

Мы используем два различных типа независимости данных: физическую и логическую. Физическая независимость - это модификация схемы на физическом уровне, не затрагивающая ее концептуально. Логическая независимость - это полная противоположность, изменение на концептуальном уровне без влияния на представление.
...
JAVASCRIPT
Транзакция Mongodb v4.0, MongoError: номера транзакций разрешены только для члена набора реплик или монго
Я установил MongoDB v4.0 для самой удивительной функции Сделка в Nodejs с mongodb 3.1 в качестве драйвера.
...
Чтобы использовать транзакции, вам понадобится набор реплик MongoDB, а запуск набора реплик локально для разработки - это сопряженный процесс. Новый run-rs npm module упрощает запуск наборов реплик. Запуск run-rs - это все, что вам нужно для запуска набора реплик, run-rs даже установит для вас правильную версию MongoDB.
...
Установите run-rs глобально с флагом npm's -g. Вы также можете перечислить run-rs в devDependencies вашего файла package.json.

npm install run-rs -g
Затем запустите run-rs с флагом --version. Run-rs загрузит для вас MongoDB v4.0.0. Не волнуйтесь, это не перезапишет вашу существующую установку MongoDB.
...
Вы найдете более подробную информацию об этом здесь.
 ...
 22.07.2018 06:22
Я борюсь с этой проблемой несколько недель. Я даю вам свой вывод. Чтобы иметь возможность использовать транзакции в сегментированном кластере, вам необходимо запустить в своем кластере как минимум MongoDB 4.2. Если кластер не сегментирован, начиная с 4.0. Я использовал библиотеку, которая имеет в качестве подзависимости драйвер NodeJS mongodb. Этот драйвер из версии 3.3.x не работает с сегментированным кластером MongoDB с версией 4.0.4. Решением для меня было обновить кластер до версии 4.2.

src: https://www.bmc.com/blogs/mongodb-transactions/

 01.10.2019 18:13
Недавно я столкнулся с той же проблемой. В моем случае это потому, что я подключаюсь к удаленному серверу Mongo с версией, отличной от моей локальной среды разработки.

Чтобы быстро решить проблему, я добавил в строку подключения следующий параметр:

?retryWrites=false

 20.02.2020 20:53
Чтобы использовать транзакции, вам понадобится набор реплик MongoDB, а запуск набора реплик локально для разработки - это сопряженный процесс.

Вы можете использовать run-rs npm module. Бегун MongoDB с нулевой конфигурацией. Запускает набор реплик без зависимостей, не связанных с узлами, даже без MongoDB.

Или вы можете просто создать учетную запись в MongoDB Atlas, которая дает вам ограниченный ресурс кластера MongoDB, и вы можете запускать / тестировать свое приложение.

Атлас MongoDB

 17.12.2020 14:46
У меня есть решение, это всего лишь конфигурация из трех строк внутри конфигурационного файла MongoDB.

После перехода с атласа MongoDB и установки MongoDB v 4.4.0 на мой CentOS 7 VPS с WHM я также столкнулся с этой проблемой.

Решение run-rs у меня не работает, но мне удалось решить эту проблему без каких-либо сторонних инструментов, выполнив следующие действия:

1. выключить mongod.
наиболее эффективный способ - войти в оболочку MongoDB с помощью команды mongo проверить метод

db.shutdownServer()
У вас не будет возможности использовать сервер MongoDB. Для меня процесс выключения занял слишком много времени, а затем я убил его с помощью команды:

systemctl stop -f mongod
если вы убили процесс mongod, возможно, вам нужно будет запустить mongod --dbpath /var/db --repair

var/db должен указывать на каталог вашей базы данных.

2. настройка конфигурации ReplicaSet.
для шага настроек replicaSet проверьте файл /etc/mongod.conf, найдите строку значений replication, и вы должны добавить следующие строки, как показано ниже:

replication:
   oplogSizeMB: <int>
   replSetName: <string>
   enableMajorityReadConcern: <boolean>
используйте значение replSetName на следующем шаге.

3. добавьте URL-адрес строки подключения.
добавьте значение replSetName к URL-адресу вашего подключения &replicaSet=--YourReplicationSetName--

4. снова включите mongod
введите команду: systemctl start mongod

5. Получите доступ к базе данных ReplicaSet.
войдите в оболочку MongoDB с командой mongo, введите команду rs.initiate() теперь вы должны быть в своей базе данных replicaSet.
